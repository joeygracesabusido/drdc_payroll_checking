...
class Payrollcomputation():
    ...
    def excel_connection_1st_cut_off(): # this function is for connectio of excel file data of 1st cut-off
        ...
        # data_df_2ND_batch = pd.read_excel(r'C:\Users\Jerome\Desktop\Payrollcomp\DRDC.xlsx',sheet_name=sheet_name)
    ...
    def monthly_computation(): # this function is for monthly computation
        ...
        result_data = merged_data[['EMPLOYEE_ID', 'COMPANY_x',
                                    'SEMI_MONTHLY_RATE_x', 'GROSS_PAY_x', 'GROSS_PAY_y']].groupby(['EMPLOYEE_ID', 'COMPANY_x']).agg({
        ...
        result_data['TOTAL_GROSS_PAY'] = result_data['GROSS_PAY_x'] + result_data['GROSS_PAY_y']
        ...
        for index, row_sss in sssData.iterrows():
            ...
            in_range = (result_data['TOTAL_GROSS_PAY'] >= row_sss['Rate_from']) & (result_data['TOTAL_GROSS_PAY'] <= row_sss['Rate_to'])
        ...
        # Handle PHIC calculation
        result_data['PHIC'] = np.where(result_data['TOTAL_GROSS_PAY'] <= 10000, 500 / 2, 0)
        result_data['PHIC'] += np.where((result_data['TOTAL_GROSS_PAY'] > 10000) &
                                        (result_data['TOTAL_GROSS_PAY'] < 100000.01),
                                        result_data['TOTAL_GROSS_PAY'] * 0.05 / 2, 0)
        
        result_data['NET TAXABLE'] = result_data['TOTAL_GROSS_PAY'] - result_data['Employee Share'] - result_data['PHIC']
        ...
        def calculate_tax(net_taxable_income):
            ...
            tax[mask_1] = net_taxable_income[mask_1] * 0
            tax[mask_2] = (net_taxable_income[mask_2] - 20833) * 0.15
            tax[mask_3] = (net_taxable_income[mask_3] - 33332) * 0.20 + 1875
        ...
        result_data = result_data[['EMPLOYEE_ID', 'COMPANY_x', 'TOTAL_GROSS_PAY',
                                    'Employee Share', 'Employer Share', 'ECC-REMT', 'PHIC','NET TAXABLE','TAX WITHHELD']]
